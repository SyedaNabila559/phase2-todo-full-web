...
export function UserMenu({ className }: UserMenuProps) {
  ...
  // Check authentication on mount & when localStorage changes
  useEffect(() => {
    const checkAuth = () => {
    ...
    <div className={cn("flex items-center gap-4", className)}>
      {userName && (
        <div className="hidden md:flex flex-col items-end">
          ...
          <span className="text-sm font-bold text-transparent bg-clip-text bg-gradient-to-r from-indigo-500 to-purple-600 dark:from-indigo-400 dark:to-purple-400">
            {userName}
          </span>
        </div>
      )}
